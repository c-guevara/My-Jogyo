...
import { fileExists, copyFileNoFollow } from "../lib/atomic-write";
  ...
  async execute(args) {
    switch (args.action) {
      case "scan": {
        ...
        if (args.reportTitle) {
          filteredViolations = violations.filter(
            (v) => v.reportTitle === args.reportTitle
          );
        }
      ...
      case "fix": {
        ...
        const result = await fixViolations(violations, dryRun, args.reportTitle);
      ...
      case "validate": {
        const { valid, issues } = await validateStructure(args.reportTitle);
        ...
        return JSON.stringify(
          {
            ...
            message: valid
              ? args.reportTitle
                ? `Structure is valid for report '${args.reportTitle}'.`
