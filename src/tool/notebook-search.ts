/**
 * Notebook Search Tool - Full-text search within Jupyter notebooks for Gyoshu research.
 *
...
function searchCell(
  ...
  if (includeOutputs && cell.cell_type === "code" && cell.outputs) {
    const outputTexts = extractOutputText(cell.outputs);
...
/**
 * Get all research projects (legacy).
 */
async function getAllResearchIds(): Promise<string[]> {
  const researchDir = getResearchDir();
