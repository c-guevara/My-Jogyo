/**
 * Migration Tool - OpenCode tool for migrating legacy Gyoshu sessions
 *
          ...
          return JSON.stringify(
            {
              ...
              message: result.skipped
                ? `Session '${args.sessionId}' was already migrated`
                : result.success
                  ? `Session '${args.sessionId}' migrated successfully${dryRun ? " (dry run)" : ""}`
                  : `Failed to migrate session '${args.sessionId}': ${result.error}`,
            },
            null,
            2
          );
          ...
          return JSON.stringify(
            {
              ...
              message: result.valid
                ? `Migration verified successfully for session '${args.sessionId}'`
                : `Verification failed for session '${args.sessionId}': ${result.issues.join(", ")}`,
            },
            null,
            2
          );
          ...
          return JSON.stringify(
            {
              ...
              totalFailed: result.success ? 0 : 1,
              message: result.skipped
                ? `Research '${args.sessionId}' was already migrated to notebooks/`
                : result.success
                  ? `Research '${args.sessionId}' migrated successfully to notebooks/${dryRun ? " (dry run)" : ""}`
                  : `Failed to migrate research '${args.sessionId}': ${result.error}`,
            },
            null,
            2
          );
