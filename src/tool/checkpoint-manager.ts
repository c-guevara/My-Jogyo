/**
 * Checkpoint Manager Tool - Manages checkpoints for Gyoshu research resume capability
 *
...
/**
 * Calculate SHA256 hash of a file using streaming (handles large files).
 * Uses openNoFollow to prevent symlink following (TOCTOU-safe).
 *
...
export const checkpointManager = tool({
  ...
  args: {
    ...
    runId: tool.schema
      .string()
      .optional()
      .describe("Run identifier (required for save/validate, optional for list/resume/prune)"),
    checkpointId: tool.schema
      .string()
      .optional()
      .describe("Checkpoint identifier (required for save/validate)"),

    // Save action parameters
    researchSessionID: tool.schema
      .string()
      .optional()
      .describe("Research session ID (required for save)"),
    stageId: tool.schema
      .string()
      .optional()
      .describe("Stage ID this checkpoint was created after, e.g. S01_load_data (required for save)"),
    status: tool.schema
      .enum(["saved", "interrupted", "emergency"])
      .optional()
      .describe("Checkpoint status (default: saved)"),
    reason: tool.schema
      .enum(["timeout", "abort", "error"])
      .optional()
      .describe("Reason for emergency checkpoint (required when status=emergency)"),
    trustLevel: tool.schema
      .enum(["local", "imported", "untrusted"])
      .optional()
      .describe("Trust level for checkpoint (default: local)"),
    executionCount: tool.schema
      .number()
      .optional()
      .describe("REPL execution count at checkpoint time"),
    notebookPathOverride: tool.schema
      .string()
      .optional()
      .describe("Override notebook path (defaults to notebooks/{reportTitle}.ipynb)"),
    pythonEnv: tool.schema
      .object({
        pythonPath: tool.schema.string(),
        packages: tool.schema.array(tool.schema.string()),
        platform: tool.schema.string(),
      })
      .optional()
      .describe("Python environment metadata for reproducibility"),
    artifacts: tool.schema
      .array(
        tool.schema.object({
          relativePath: tool.schema.string(),
          sha256: tool.schema.string(),
          sizeBytes: tool.schema.number(),
        })
      )
      .optional()
      .describe("Array of artifact entries with paths and integrity hashes"),
    rehydrationMode: tool.schema
      .enum(["artifacts_only", "with_vars"])
      .optional()
      .describe("Rehydration mode (default: artifacts_only)"),
    rehydrationSource: tool.schema
      .array(tool.schema.string())
      .optional()
      .describe("Custom rehydration cell source lines (auto-generated if not provided)"),

    // Prune action parameters
    keepCount: tool.schema
      .number()
      .optional()
      .describe("Number of checkpoints to keep when pruning (default: 5)"),
  },

  async execute(args) {
    const { action, reportTitle, runId, checkpointId } = args;
    ...
    switch (action) {
      ...
      case "save": {
        ...
        const status: CheckpointStatus = args.status || "saved";

        // Validate emergency checkpoints have a reason
        if (status === "emergency" && !args.reason) {
        ...
        const artifacts: ArtifactEntry[] = args.artifacts || [];

        // Build python env metadata
        const pythonEnv: PythonEnvMetadata = args.pythonEnv || {
          pythonPath: "unknown",
          packages: [],
        ...
        const rehydrationMode: RehydrationMode =
          args.rehydrationMode || "artifacts_only";
        const rehydrationCellSource =
          args.rehydrationSource ||
        ...
        const manifestBase: Omit<CheckpointManifest, "manifestSha256"> = {
          ...
          status,
          ...(status === "emergency" && args.reason
            ? { reason: args.reason as EmergencyReason }
            : {}),
          trustLevel: (args.trustLevel as TrustLevel) || "local",
          createdAt: new Date().toISOString(),
          executionCount: args.executionCount || 0,
          notebook: {
            path: path.relative(projectRoot, notebookPath),
            checkpointCellId: "", // Will be updated after appending cell
          },
          pythonEnv,
          artifacts,
          rehydration: {
      ...
      case "prune": {
        ...
        const keepCount = args.keepCount || DEFAULT_KEEP_COUNT;
      ...
      case "emergency": {
        ...
        const manifestBase: Omit<CheckpointManifest, "manifestSha256"> = {
          ...
          researchSessionID: args.researchSessionID || "unknown",
          reportTitle,
          runId,
          stageId: args.stageId,
          ...
          notebook: {
            path: path.relative(projectRoot, notebookPath),
            checkpointCellId: "", // May be updated if cell append succeeds
          },
          pythonEnv,
          artifacts,
          rehydration: {
